...
export class DocumentsEndpoint extends EndpointFactory {
    ...
    search(filter: string, value: string, grpId: string, isPrintTemplate: number, page: number, size: number): Observable<Response> {

        if (grpId == 'undefined') grpId = '';
        if (value == 'undefined') value = '';

        let url = this.getUrl('search?filter=' + filter
            + '&value=' + value
            + '&grpId=' + grpId 
        ...
        return this.http.get(url, this.getAuthHeader())
            ...
            .catch(error => {
                return this.handleError(error, () => this.search(filter, value, grpId, isPrintTemplate, page, size));
            });
    }
